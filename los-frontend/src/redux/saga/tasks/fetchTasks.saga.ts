...
import { FETCH_TASKS_DATA } from '@/redux/actions/types';
...
function* fetchTasksSaga(action: {
  ...
  try {
    ...
    const response: ApiResponse<{ data: ITaskTable[]; total: number }> = yield call(
      apiCaller,
      `/tasks?active=${activeTab === 'active'}&activeFilter=${action.payload?.filter?.activeFilter || activeFilter}&statusFilter=${action.payload?.filter?.statusFilter || statusFilter}&page=${activePage}`
    );
...
export default function* fetchTasks() {
  yield takeEvery(FETCH_TASKS_DATA, fetchTasksSaga);
}
