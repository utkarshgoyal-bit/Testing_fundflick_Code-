...
function* addCustomerIncomeSaga(action: ISagaProps) {
  ...
  try {
    ...
    if (response.data) {
      yield put(setActiveStep(STEPS_NAMES.LIABILITY));
    }
  } catch (error) {
...
export default function* addCustomerIncome() {
  yield takeEvery(ADD_CUSTOMER_INCOME_DATA, addCustomerIncomeSaga);
}
