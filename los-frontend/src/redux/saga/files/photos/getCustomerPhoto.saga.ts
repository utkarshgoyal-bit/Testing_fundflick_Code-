...
import { setData, setError } from '@/redux/slices/files/photos';
import { setLoading } from '@/redux/slices/publicSlice';
import { call, put, takeEvery } from 'redux-saga/effects';

function* getCustomerPhotosSaga() {
  yield put(setLoading({ loading: true }));
  const urlParams = new URLSearchParams(window.location.search);
  const id = urlParams.get('id');
  try {
    const response: ApiResponse<any> = yield call(apiCaller, `/customer-file/customer_photos/${id}`, 'GET', {}, false);
    if (response.data) {
...
export default function* getCustomerPhotos() {
  yield takeEvery(GET_CUSTOMER_PHOTOS_DATA, getCustomerPhotosSaga);
}
