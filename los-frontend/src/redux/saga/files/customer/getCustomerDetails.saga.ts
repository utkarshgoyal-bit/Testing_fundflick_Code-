...
import { GET_CUSTOMER_DETAILS_DATA } from '@/redux/actions/types';
import { setStepsData } from '@/redux/slices/files';
import { setData, setError } from '@/redux/slices/files/customerDetails';
...
function* getCustomerDetailsSaga() {
  ...
  try {
    const response: ApiResponse<any> = yield call(apiCaller, `/customer-file/customer_details/${id}`, 'GET', {}, false);
    if (response.data) {
...
export default function* addCustomerDetails() {
  yield takeEvery(GET_CUSTOMER_DETAILS_DATA, getCustomerDetailsSaga);
}
