...
import { GET_CUSTOMER_ADDRESS_DATA } from '@/redux/actions/types';
import { setStepsData } from '@/redux/slices/files';
import { setData, setError } from '@/redux/slices/files/address';
...
function* getCustomerAddressSaga() {
  ...
  try {
    const response: ApiResponse<any> = yield call(apiCaller, `/customer-file/customer_address/${id}`, 'GET', {}, false);
    if (response.data) {
...
export default function* getCustomerAddress() {
  yield takeEvery(GET_CUSTOMER_ADDRESS_DATA, getCustomerAddressSaga);
}
