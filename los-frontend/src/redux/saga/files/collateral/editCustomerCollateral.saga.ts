...
import { EDIT_CUSTOMER_COLLATERAL_DATA } from '@/redux/actions/types';
import { setActiveStep } from '@/redux/slices/files';
import { setError } from '@/redux/slices/files/collateral';
import { setLoading } from '@/redux/slices/publicSlice';
import { call, put, takeEvery } from 'redux-saga/effects';
function* editCustomerCollateralSaga(action: ISagaProps) {
  ...
  try {
    ...
    if (response.data) {
      yield put(setActiveStep(STEPS_NAMES.BANK));
    }
  } catch (error) {
...
export default function* editCustomerCollateral() {
  yield takeEvery(EDIT_CUSTOMER_COLLATERAL_DATA, editCustomerCollateralSaga);
}
