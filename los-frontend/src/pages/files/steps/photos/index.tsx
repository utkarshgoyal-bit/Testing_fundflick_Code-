...
import { ChevronDown, ChevronUp, FileText, Trash } from 'lucide-react';
...
const PhotoGroup = ({ heading, photoGroup }: { heading: string; photoGroup: string }) => {
  ...
  const { photos, photosAddFormDialog, loading } = useSelector((state: RootState) => state.filePhotos);
                    ...
                    <AlertDialog>
                      <AlertDialogTrigger
                        ...
                        className={cn(
                          ...
                          'w-fit hover:bg-destructive hover:text-white text-destructive'
                        )}
                      >
                        <Trash size={19} />
                      </AlertDialogTrigger>
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            This action cannot be undone. This will permanently delete this photo and remove your data
                            from our servers.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                          ...
                          <AlertDialogAction
                            onClick={() => {
                              dispatch({
                                type: DELETE_CUSTOMER_PHOTOS_DATA,
...
export default function PhotosStep() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      <h1 className="text-2xl font-bold text-gray-800 mb-6">Photo Groups</h1>
      ...
      <div className="flex col-span-2  w-full justify-end items-center gap-5  ">
        <div className="flex justify-end col-span-2 gap-4 my-3">
          <Button onClick={() => navigation(buildOrgRoute(ROUTES.CUSTOMER_FILE_MANAGEMENT))}>Save</Button>
        </div>
      </div>
    </div>
  );
