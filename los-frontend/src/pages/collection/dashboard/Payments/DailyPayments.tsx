...
const DailyPayments = ({ withoutFilters }: { withoutFilters?: boolean }) => {
  ...
  const day = searchParams.get('collectionDay');
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      cell: (props) => (
        <div className="flex items-center gap-2">
          <IndianRupee className="h-4 w-4 text-color-success" />
    ...
    },
    {
      ...
      cell: (props) => (
        <div
          className={`px-3 py-1 rounded-full text-xs font-medium border ${getPaymentModeColor(props.row.original.paymentMode)}`}
        >
