...
import convert24to12 from '@/helpers/convert24hroTo12hr';
...
const DailyFollowups = ({ withoutFilters }: { withoutFilters?: boolean }) => {
  ...
  const { filters: dashboardFilters } = useSelector((state: RootState) => state.collectionDashboard);
  ...
  const getAttitudeColor = (attitude: string) => {
    switch (attitude?.toLowerCase()) {
      case 'polite':
        return 'bg-color-success text-white  border-color-success/20';
      case 'rude':
        return 'bg-color-error/10 text-color-error border-color-error/20';
      case 'medium':
        return 'bg-color-warning  text-white border-color-warning/20';
      case 'noreply':
        return 'bg-color-surface text-fg-secondary border-fg-border';
      default:
        return 'bg-color-surface text-fg-secondary border-fg-border';
    }
  };
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      cell: (props) => (
        <div
          className={`px-3 py-1 flex justify-center rounded-full text-xs font-medium border ${getAttitudeColor(props.row.original.attitude)}`}
        >
          {props?.row?.original?.attitude?.toUpperCase()}
        </div>
      ),
    },
    {
      ...
      cell: (props) => (
        <div className="flex items-center gap-2">
          ...
          <span className="text-fg-primary text-sm">
            {formatDate(props.row.original?.createdAt)} {convert24to12(props.row.original?.createdAt.split('T')[1])}
          </span>
        </div>
      ),
    },
    ...
    },
    {
      ...
      cell: (props) => (
        <div
          className={`px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(props.row.original.commitStatus)}`}
        >
          {statusTitle[props.row.original.commitStatus as keyof typeof statusTitle]}
        </div>
      ),
    },
    {
  ...
  const getCaseBgColor = (row: any) => {
    ...
    const selectedDay = dashboardFilters['followupDay'];
    const getCaseBg: any = {
      [COMMIT_STATUS_ORDER.expired]: 'bg-red-300 hover:bg-red-300',
      [COMMIT_STATUS_ORDER.pending]: 'bg-yellow-300 hover:bg-white-300',
      [COMMIT_STATUS_ORDER.fulfilled]: 'bg-green-400 hover:bg-white-400',
    };
    if (commitStatus == COMMIT_STATUS_ORDER.fulfilled && row.refCaseId.dueEmi > 0) {
      return 'bg-white';
    }
  ...
  return (
    <div className="space-y-6 ">
      ...
      {/* Data Table */}
      {!!data?.length && (
        <div className=" border border-fg-border rounded-lg overflow-hidden">
          <RTable
