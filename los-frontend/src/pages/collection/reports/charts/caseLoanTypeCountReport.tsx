...
function generateDistinctColors(count: number): string[] {
  ...
  for (let i = 0; i < count; i++) {
    const hue = Math.round((360 / count) * i + 30); // Evenly spaced hues
...
export function LoanTypeCaseCountReport() {
  const { data } = useSelector((state: RootState) => state.collectionDashboardReport);

  const chartData = data?.statesData.loanTypeSummary || [];
  ...
  return (
    <Card className="flex flex-col w-full">
      <CardHeader className="items-center pb-0">
        <CardTitle>Loan type wise cases - Pie Chart</CardTitle>
        ...
        {data?.collectionReportsData.length > 0 ? (
          <ChartContainer config={chartConfig} className="mx-auto h-[350px] w-[350px]">
            <PieChart>
              ...
              <Pie data={mappedChartData} dataKey="count" nameKey="loanType" />
            </PieChart>
          </ChartContainer>
        ) : (
          <div>
      ...
      <CardFooter className="flex-col gap-2 text-sm">
        <div className="flex items-center gap-2 leading-none font-medium">
          {mappedChartData.length} Loan Types
