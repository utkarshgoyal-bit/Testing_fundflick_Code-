...
export default function CollectionTable<TData, TValue>({
  ...
  return (
    <div className="rounded-lg px-10 border w-full relative">
          ...
          {table.getRowModel().rows?.length ? (
            table.getRowModel().rows.map((row: any) => (
              <TableRow
                ...
                className={cn(
                  'cursor-pointer',
                  `${row?.original?.dueEmiAmount === 0 && row?.original?.dueEmi === 0 ? 'bg-green-500 hover:bg-green-500' : ''}  ${getRowClass && getRowClass(row.original)}`
                )}
              >
      ...
      <div className="w-full p-3 border-t-2">
        {customFooter ? (
          ...
          <div className="w-full flex justify-between items-center ">
            ...
            <div>
              <Button variant="outline" size="sm" onClick={handlePreviousPage} disabled={page <= 0} className="mr-2">
                Previous
              </Button>
              <Button variant="outline" size="sm" onClick={handleNextPage} disabled={page + 1 >= table.getPageCount()}>
                Next
              </Button>
            </div>
          </div>
