...
import { MONTHS, WEEK_DAYS, TASK_TYPES, PAYMENT_TYPES } from '@/constants/constants';
...
export function NewTask({
  ...
  const form = useForm<ITaskFormType>({
    ...
    defaultValues: task
      ? {
          ...
          users: task.users || [],
          type: task.type || isCAType ? 'CA' : 'pendency',
  ...
  useEffect(() => {
    const payload: any = { isBlocked: false, isFetchClients: isCAType, isAllowSelfUser: true };
    dispatch({ type: FETCH_USERS_DATA, payload });
  }, [dispatch]);

  useEffect(() => {
    form.setValue('type', isCAType ? 'CA' : 'pendency');
  }, [isCAType]);
