...
const hasPermission = (permissionName?: string) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const user = await UserModel.findOne({ employeeId: new Types.ObjectId(employeeId) }).populate<{
        roleRef: { permissions: string[] };
      }>("roleRef");
      if (isSuperAdmin([user?.role || ""]) || (permissionName && user?.roleRef.permissions.includes(permissionName))) {
        return next();
      } else {
        return ApiResponseHandler.sendErrorResponse(res, ERROR.INVALID_ROLE, ERROR.INVALID_ROLE);
      }
    } catch (error) {
      return ApiResponseHandler.sendErrorResponse(res, ERROR.INTERNAL_SERVER_ERROR, ERROR.INTERNAL_SERVER_ERROR);
    }
  };
};
