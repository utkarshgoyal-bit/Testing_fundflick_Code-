...
const getFiles = async ({
  ...
  if (_canViewSelf && !_isSuperAdmin && !_canViewOthers && !_canViewBranch) {
    query = {
      ...query,
      ...(query.createdBy && { createdBy: new Types.ObjectId(loginUser?.employeeId) }),
  ...
  if (_canViewBranch && !_isSuperAdmin && !_canViewOthers) {
    ...
    query = {
      ...query,
      $or: [
        { createdBy: new Types.ObjectId(loginUser?._id) },
        { fileBranch: { $in: loginUser?.branches } },
      ],
    };
  }
  if (filters?.loanApplicationNumber) {
    query = {
      ...
      ...(query.loanApplicationNumber && {
        loanApplicationNumber: +filters?.loanApplicationNumber,
      }),
    };
  }
