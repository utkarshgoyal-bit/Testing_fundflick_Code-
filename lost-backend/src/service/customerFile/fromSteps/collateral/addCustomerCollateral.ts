...
const addCustomerCollateral = async ({ fileId, body, loginUser }: { fileId: string; body: any; loginUser: any }) => {
  ...
  let responseUpdatedCollateralDetails = updatedCustomerFile.toObject().collateralDetails.map((collateral: any) => {
    if (collateral.collateralType === "vehicle" && collateral?.vehicleDetails?.vehicleDetails) {
      const { _id, createdAt, createdBy, updatedAt, _v, ...rest } = collateral.vehicleDetails.vehicleDetails as any;
      const { vehicleDetails, ...rest1 } = collateral.vehicleDetails as any;
  ...
  const customerOtherFamilyDetails = updatedCustomerFile?.toObject()?.customerOtherFamilyDetails.map((item: any) => {
    let { customerDetails, ...rest } = item;
    let { _id, ...customerRestDetails } = customerDetails;
