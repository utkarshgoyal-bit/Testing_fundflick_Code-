...
import CollectionRevisions from '../../../schema/collectionRevisions/dataModel';
...
const rawRowSchema = z.array(
  z.object({
    ...
    'Due EMI Amt': z.coerce
      .number()
      .int('Due EMI Amt should be a number')
      .min(1, 'Due EMI Amt is required'),
...
const cleanupAndRevisions = async (
  ...
  const collectionOps = collections.map(doc => ({
    insertOne: {
      document: {
        ...doc,
        revisionId,
  ...
  const followUpOps = followUps.map(doc => ({
    insertOne: {
      document: {
        ...doc,
        revisionId,
  ...
  const paymentOps = payments.map(doc => ({
    insertOne: {
      document: {
        ...doc,
        revisionId,
...
const uploadCasesCollection = async ({
  ...
  const casesToUpdate = Array.from(existingCaseMap.keys()).filter(
    caseNo => !casesInFile.has(caseNo)
  );
  ...
  if (allBulkOps.length > 0) {
    try {
      ...
      throw new Error('Bulk write error: ' + (error.message || error.toString()));
    }
  }
};
